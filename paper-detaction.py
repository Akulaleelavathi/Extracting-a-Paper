...
# # Apply GaussianBlur to reduce noise and improve edge detection (increased kernel size)
...
# # Use the largest contour, even if it doesn't have four corners
# largest_contour = contours[0]

# # Get the minimum enclosing rectangle for the largest contour
...
# # Apply the order points function
# ordered_box = order_points(box)

# # Compute the width and height of the new image
# widthA = np.sqrt(((ordered_box[2][0] - ordered_box[3][0]) ** 2) + ((ordered_box[2][1] - ordered_box[3][1]) ** 2))
# widthB = np.sqrt(((ordered_box[1][0] - ordered_box[0][0]) ** 2) + ((ordered_box[1][1] - ordered_box[0][1]) ** 2))
# maxWidth = max(int(widthA), int(widthB))

# heightA = np.sqrt(((ordered_box[1][0] - ordered_box[2][0]) ** 2) + ((ordered_box[1][1] - ordered_box[2][1]) ** 2))
# heightB = np.sqrt(((ordered_box[0][0] - ordered_box[3][0]) ** 2) + ((ordered_box[0][1] - ordered_box[3][1]) ** 2))
# maxHeight = max(int(heightA), int(heightB))

# # Set destination points to obtain a "birds eye view" of the image
# dst = np.array([
#     [0, 0],
#     [maxWidth - 1, 0],
